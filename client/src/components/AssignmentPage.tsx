...
export default function AssignmentPage() {
    ...
    const result = {
        ...
        "detailed_feedback": {
          ...
          "question_details": [
            {
              ...
              "feedback": "Accurate setup but final answer missing units,Accurate setup but final answer missing unitsAccurate setup but final answer missing unitsAccurate setup but final answer missing unitsAccurate setup but final answer missing unitsAccurate setup but final answer missing unitsAccurate setup but final answer missing unitsAccurate setup but final answer missing unitsAccurate setup but final answer missing unitsAccurate setup but final answer missing unitsAccurate setup but final answer missing unitsAccurate setup but final answer missing unitsAccurate setup but final answer missing unitsAccurate setup but final answer missing unitsAccurate setup but final answer missing unitsAccurate setup but final answer missing unitsAccurate setup but final answer missing unitsAccurate setup but final answer missing unitsAccurate setup but final answer missing unitsAccurate setup but final answer missing unitsAccurate setup but final answer missing unitsAccurate setup but final answer missing unitsAccurate setup but final answer missing unitsAccurate setup but final answer missing unitsAccurate setup but final answer missing unitsAccurate setup but final answer missing unitsAccurate setup but final answer missing units"
            },
            {
              "question_id": "Q5",
              "score": 2.0,
          ...
          "rubric_summary": "Grading criteria: 40% accuracy, 30% methodology, 20% presentation, 10% timeliness. Level descriptors: Excellent (9-10 pts): Exceeds requirements; Good (7-8.9 pts): Meets all requirements; Needs Work (<7 pts): Missing key elements.",
          "actual_summary": "Your submission shows strong technical knowledge but lacks attention to formatting requirements. While your calculations are mostly correct (scoring 92% on accuracy), presentation errors cost 15% of total marks. Focus on citation formatting and showing complete working steps to reach A-grade level."
        }
      };
      const students = [
        ...
        <div  className=" flex flex-col  bg-[#F2F4F8] w-full h-screen">
            ...
            <div className="flex flex-row items-center justify-center h-full ">
                ...
                <div className="w-[48rem] h-[38rem] mx-auto bg-white rounded-xl shadow-md overflow-hidden hover:shadow-lg transition-shadow duration-300">
                    <div className="p-6">
                        <h2 className="text-3xl font-bold text-gray-900 mt-3 text-center">{title}</h2>
                        ...
                        <div>
                            <h3 className="font-bold text-lg text-gray-800 ml-4 mt-2">Files:</h3>
                            <p className="text-gray-700 ml-6">{getter}</p>
                        </div>

                        {!isTeacher && <div className="flex items-center ml-4 mt-4">
                            <span className="font-bold text-lg text-gray-800">Status:</span>
                            <span className={`ml-2 px-2 py-1 text-sm ${status === 'Completed' ? 'text-green-500 bg-green-100' : 'text-red-500 bg-red-100'
                    } text-sm rounded-full` }>{status}</span>
                        </div>}
                    </div>
                    ...
                    <div className="w-[24rem] h-[10rem] mx-auto bg-red-50 rounded-xl shadow-md overflow-hidden hover:shadow-lg transition-shadow duration-300">
                        <div className="transition-all duration-300 ease-in-out">
                            <p className="text-red-800 text-3xl text-center mt-8">Result is not out yet.</p>
                            <p className="text-red-800 text-xl text-center mt-8">Time: 3 days</p>
                        </div>
                    </div>
                                    ...
                                    <ul className="space-y-4 overflow-y-auto" >
                                        ...
                                        <li
                                            ...
                                            <div className="flex justify-between items-center">
                                                <p className="text-xl font-semibold">{student.name} 👉🏻
                                                    <span className="text-xl font-semibold ml-2">{student.marks}/100</span>
                                                </p>
                                            </div>
                                        </li>
                                        ))}
                        ...
                        <div className="transition-all duration-300 ease-in-out">
                            <div className=" p-4 rounded-lg">
                                <div className="text-green-800 text-4xl text-center">Result is out!</div>
                                <p className="text-green-700 text-2xl mt-4 ml-4">
                                Your score: <span className="font-bold">{result.score}/100</span>
                                </p>
                            </div>
                            ...
                            <div className=" mt-2 p-8 border-t border-gray-200 ">
                                ...
                                {result.summary_bullets.map((bullet, index) => (
                                    <li key={index} className=" ml-2 mt-2 text-xl text-gray-700">{bullet}</li>
                                ))}
                                </ul>
                            </div>

                            <Link onClick={() => dispatch(isSidebarState(true))}
                                to="result-feedback"
                                state= {result}>
                                <div 
                                    className="justify-self-center w-[18rem] bg-[#aab2c6] text-center text-black px-4 py-2 rounded-lg cursor-pointer hover:bg-[#8591ad] text-xl transition duration-300">
